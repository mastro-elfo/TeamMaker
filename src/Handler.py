...
class Handler(Builder):
    ...
    def on_ConfirmNewPlayer_clicked(self, *args):
        ...
        self.get_object("PlayersStore").append(
            (
                player.id,
                player.nickname,
                player.number,
    ...
    def on_ConfirmEditPlayer_clicked(self, *args):
        ...
        if path is not None:
            ...
            if player is not None:
                player.update(
                    **dict(
                        zip(("nickname", "number", "rating", "name", "surname"), data)
                    )
                )
                self.players.dump(PLAYERS_PICKLE)
    ...
    def on_MakePopover_show(self, *args):
        ...
        self.update_store(
            "MakeLeftStore",
            map(
                lambda x: (x.id, x.nickname, x.number, x.rating, x.name, x.surname),
                left_team,
            ),
        )
        self.update_store(
    ...
    def on_MakeLeftRightStore_row_changed(self, *args):
        left_team = Players(
            *[self.players.find_id(p[0]) for p in self.get_object("MakeLeftStore")]
        )
        right_team = Players(
            *[self.players.find_id(p[0]) for p in self.get_object("MakeRightStore")]
        )
        self.set_text("LeftTeamStrength", str(left_team.strength()))
        self.set_text(
            "LeftTeamChance", str(int(left_team.chance(right_team) * 100)) + "%"
        )
        self.set_text("RightTeamStrength", str(right_team.strength()))
        self.set_text(
            "RightTeamChance", str(int(right_team.chance(left_team) * 100)) + "%"
        )
    ...
    def on_ConfirmDoUpdates_clicked(self, button, *args):
        ...
        if self.__latest is None:
            self.set_text(
                "UpdatesStatus", "Unexpected error while updating, nothing changed"
            )
            return
