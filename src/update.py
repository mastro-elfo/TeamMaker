...
def extract_update():
    try:
        unpack_archive(
            join((abspath(sep)), "tmp", "teammaker.tar.gz"),
            join((abspath(sep)), "tmp", "teammaker"),
        )
        return True, ""
    except Exception as e:
        return False, str(e)
...
def install_contents():
    try:
        ...
        rmtree(
            join(expanduser("~"), ".local", "share", "TeamMaker"), ignore_errors=True
        )
        copytree(
            join((abspath(sep)), "tmp", "teammaker", pack_dir),
            join(expanduser("~"), ".local", "share", "TeamMaker"),
        )
        return True, ""
    except Exception as e:
        return False, str(e)
...
def get_version(tag):
    return tuple(
        [
            int(x)
            for x in tag.translate(
                str.maketrans("", "", ascii_letters + punctuation.replace(".", ""))
            ).split(".")
        ]
    )
